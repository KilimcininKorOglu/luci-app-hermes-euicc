...
function loadProfiles() {
    var el = document.getElementById('profiles-loading'); if (el) { el.classList.remove('hidden'); el.style.display = 'block'; }
        ...
        if (xhr.readyState === 4) {
            ...
            if (xhr.status === 200) {
                ...
                if (data.success) {
                    ...
                    var el = document.getElementById('profiles-content'); if (el) { el.classList.remove('hidden'); el.style.display = 'block'; }
...
function displayProfiles(profiles) {
    ...
    profiles.forEach(function (profile) {
        ...
        } else if (profile.profile_name && profile.profile_name.trim() !== '') {
        ...
        var statusValue = profile.profile_state;
        var statusLabel = '';
        var statusClass = '';

        // hermes-euicc returns: 0=disabled, 1=enabled
        if (statusValue === 1) {
...
function showChangeNameDialog(iccid, currentName) {
    ...
    if (currentProfile) {
        if (currentProfile.profile_nickname && currentProfile.profile_nickname.trim() !== '') {
            currentDisplayName = currentProfile.profile_nickname;
        } else if (currentProfile.profile_name && currentProfile.profile_name.trim() !== '') {
            currentDisplayName = currentProfile.profile_name;
        } else if (currentProfile.service_provider_name && currentProfile.service_provider_name.trim() !== '') {
